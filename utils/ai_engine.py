...
def generate_ai_decision(lane_data, prediction_data, history):
    ...
    trend_value = 0
    if len(lane_hist) >= 10:
        trend_value = (sum(lane_hist[:5]) - sum(lane_hist[5:])) / 5

    # 3. SCORE COMPUTATION
    # FIX 4: Weights now sum to 1.0 (was 0.5 + 0.3 + 0.2 + 0.4 = 1.4)
    # New split: density 0.45 · prediction 0.25 · trend 0.15 · fairness 0.15
    for lane in lane_data:
        density_score  = lane_data[lane] * 1.0
        pred_score     = clamp(max(0, prediction_data[lane]["change"]) * 2.0, 0, 100)
    ...
    if   7  <= hour <= 9:  time_context = "morning rush hour"
    elif 17 <= hour <= 19: time_context = "evening rush hour"
    elif 12 <= hour <= 14: time_context = "midday peak"
    elif 22 <= hour or hour <= 5: time_context = "low-traffic hours"
    else:                  time_context = "off-peak period"
    ...
    return {
        ...
        "explanation": {
            ...
            "alternative_lanes": [lane for lane in lane_data if lane != best_lane],
        },
    }


def generate_prediction_text(lane_data, lane_predictions):
    """Overall prediction using current density AND forecast trends."""
